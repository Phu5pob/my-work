          ...
          {courses.map((course) => (
            <details key={course.id} className="border border-line rounded-md bg-white">
              <summary className="cursor-pointer list-none p-5 flex items-center justify-between gap-4">
                <div>
                  <p className="font-medium">
                    {course.title}{" "}
                    {!course.isActive ? (
                      <span className="text-xs text-ink-soft">(ปิดรับสมัคร)</span>
                    ) : null}
                  </p>
                  <p className="text-xs text-ink-soft mt-1">
                    {course.branch?.name} · {course.instrument} ·{" "}
                    {LEVEL_LABELS[course.level]} · {formatCurrency(course.price)}
                  </p>
                </div>
              ...
              <div className="p-5 pt-0 space-y-6">
                ...
                <div className="flex gap-4 pt-2 border-t border-line pt-4">
                  <form action={toggleCourseActiveAction}>
                    ...
                    <button className="text-sm text-ink-soft hover:text-burgundy">
                      {course.isActive ? "ปิดรับสมัครคอร์สนี้" : "เปิดรับสมัครอีกครั้ง"}
                    </button>
                  </form>
                  ...
                  {course.schedules.length > 0 ? (
                    <div className="space-y-2 mb-4">
                      {course.schedules.map((s) => (
                        <div
                          key={s.id}
                          className="flex items-center justify-between text-sm bg-ivory-deep rounded-md px-3 py-2"
                        >
                          <span>
                            วัน{dayOfWeekLabel(s.dayOfWeek)} {s.startTime}–{s.endTime} น. ·{" "}
                            {s.teacherName} · {s.room} · รับสูงสุด {s.maxStudents} คน
                          </span>
